...
class ModelTests(TestCase):
    ...
    def test_borrow_manager_is_borrowed_by_user(self):
        self.assertFalse(Borrow.objects.is_borrowed_by_user(self.user, self.book))
        Borrow.objects.create(user=self.user, book=self.book, status=Borrow.Status.open)
        self.assertTrue(Borrow.objects.is_borrowed_by_user(self.user, self.book))

    def test_borrow_save_stock_logic(self):
        # Test stock decreases when status changes from open to issued
        borrow = Borrow.objects.create(user=self.user, book=self.book, status=Borrow.Status.open)
        self.assertEqual(self.book.stock, 5)
...
class AdminTests(TestCase):
    ...
    def test_book_list_filter(self):
        ...
        # Filter for in stock
        filter_in_stock = BookListFilter(request, {'stock': '1'}, Book, BookAdmin)
        ...
        filter_out_of_stock = BookListFilter(request, {'stock': '0'}, Book, BookAdmin)
    ...
    def test_borrow_admin_actions(self):
        borrow = Borrow.objects.create(user=self.user, book=self.book_in_stock, status=Borrow.Status.open)
        ...
        borrow_admin.make_approve_borrow(request, Borrow.objects.filter(pk=borrow.pk))
        ...
        borrow_admin.make_renew_borrow(request, Borrow.objects.filter(pk=borrow.pk))
        borrow.refresh_from_db()
        self.assertEqual(borrow.status, Borrow.Status.renewed)
        self.assertIn('renewed on', borrow.notes)

        # Test make_return_borrow
        borrow_admin.make_return_borrow(request, Borrow.objects.filter(pk=borrow.pk))
        borrow.refresh_from_db()
    ...
    def test_borrow_admin_permissions(self):
        ...
        regular_user = User.objects.get(pk=regular_user.pk) # Refresh user permissions cache
        request.user = regular_user

        # Now should have permissions
        # Note: BorrowAdmin uses get_permission_codename('issue', opts) which defaults to 'issue_borrow'
        # because the permission is defined in Meta.permissions as ('issue_borrow', ...)
        # Actually django's get_permission_codename('issue', opts) returns 'issue_borrow'? 
        # Let's check admin.py: codename = get_permission_codename('issue', opts)
        # By default django codename for change is 'change_borrow'.
        # Custom permissions in Borrow model are: 'issue_borrow', 'return_borrow', 'renew_borrow'.
